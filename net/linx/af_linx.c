/*
 *  Copyright (c) 2006-2015, Enea Software AB .
 *
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,12,4))
	/* Since Linux 3.12.4 we can assume that msg.msg_name has
	 * a minimum size of sizeof(struct sockaddr_storage).
	 * msg->msg_namelen will be zero nevertheless */
...
int linx_skb_create_no_copy(struct sock *to, struct sk_buff **skb)
{
	int err = 0;
		...
		uint32_t *consumed)
{
	...
	*consumed = 0;
	...
	if (linx_sk(to)->type == LINX_TYPE_REMOTE) {
		...
		if (BUF_TYPE_POOL(buffer_type))
			*consumed = 1;
		return err;
	}

	/* Add to the receivers receive queue. */
	switch(buffer_type & BUF_TYPE_MASK) {
	case BUFFER_TYPE_POOL:
		...
		*consumed = 1;
		...
		cb->signo = *((LINX_SIGSELECT *) payload);
		skb->data = payload;
		set_mem_owner(payload, to);
		break;
	case BUFFER_TYPE_KERNEL: /* fallthrough */
	case BUFFER_TYPE_USER:
...
#else
static int linx_sendmsg(struct socket *sock, struct msghdr *msg, size_t len)
#endif
{
	struct sock *sk = sock->sk;
	...
	LINX_OSBUFSIZE payload_size = iov_length(msg->msg_iter.iov, 1);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
	LINX_ASSERT(msg->msg_iter.nr_segs == 1);
#else
...
#ifdef ERRORCHECKS
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
	if (unlikely(iov_length(msg->msg_iter.iov, 1) < sizeof(LINX_SIGSELECT))) {
		linx_err("Buffer length is too short (%zu)\n",
			 iov_length(msg->msg_iter.iov, 1));
		linx_sk_unlock(sk);
		return -EINVAL;
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
	if (err >= 0) {
		iov_iter_advance(&msg->msg_iter, payload_size);
	}
#endif
...
static int linx_ioctl_legacy_receive(struct sock *sk, unsigned long arg)
{
		...
		timeo_jiffies = msecs_to_jiffies(timeo);
                skb = linx_recv_datagram(sk, flags, &err,
                                         size, &timeo_jiffies);

        } else {
		timeo_jiffies = sk->sk_rcvtimeo;
                skb = linx_recv_datagram(sk, flags, &err,
                                         size, &timeo_jiffies);
        }
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
        iov_iter_init(&iter, READ, &to, 1, size);
        err = skb_copy_datagram_iter(skb, 0, &iter, size);
#else
...
static int linx_ioctl_legacy_2_send(struct sock *sk, unsigned long arg)
{
	uint32_t from_spid, to_spid, size, sig_attr;
	uint32_t buffer_type = BUFFER_TYPE_USER;
	unsigned long buffer;
	...
	const struct linx_sndrcv_legacy_2_param __user *sndrcv =
		(struct linx_sndrcv_legacy_2_param *)arg;

	LINX_ASSERT(access_ok(VERFIFY_READ, sndrcv,
			      sizeof(struct linx_sndrcv_legacy_2_param)));
...
static int linx_ioctl_legacy_2_receive(struct sock *sk, unsigned long arg)
{
	uint32_t from_spid, size, sigselect_size, sig_attr;
	unsigned long buffer;
	struct iovec to;
	...
	if (unlikely(timeo != (uint32_t)~0)) {
		...
		timeo_jiffies = msecs_to_jiffies(sk->sk_rcvtimeo);
		skb = linx_recv_datagram(sk, flags, &err,
					 size, &timeo_jiffies);
	}
...
static int linx_ioctl_send(struct sock *sk, unsigned long arg)
{
	...
	get_user(from_spid, &sndrcv->from);
	get_user(size, &sndrcv->size);
	get_uptr(ubuffer, &sndrcv->buffer);
	get_user(sig_attr, &sndrcv->sig_attr);
	get_user(offset, &sndrcv->offset);

	if (ubuffer == 0x0) { /* this is a pool signal */
		buffer = pool_offset_to_sig(offset);
		buffer_type = BUFFER_TYPE_POOL;
	} else {
		buffer = (void *)ubuffer;
	}
...
static int linx_ioctl_receive(struct sock *sk, unsigned long arg)
{
	...
	get_user(sigselect_size, &sndrcv->sigselect_size);
	get_user(sig_attr, &sndrcv->sig_attr);
	get_user(timeo_msecs, &sndrcv->tmo);
	...
	/* Receive the first datagram of the receive queue. */
	if (unlikely(timeo_msecs != (uint32_t)~0)) {
		timeo_jiffies = msecs_to_jiffies(timeo_msecs);
		skb = linx_recv_datagram(sk, flags, &err,
					 size, &timeo_jiffies);
	...
	if (unlikely(err != 0)) {
		linx_debug(LINX_TRACEGROUP_AF_LINX,
			   ...
			   "err=%d.", err);
		if (err == -EINTR) {
			put_user(jiffies_to_msecs(timeo_jiffies),
				(uint32_t *)&sndrcv->tmo);
		}
		goto out;
	}
	...
	case 2:
	{
		u32 offset, real_size;
		offset = pool_sig_to_offset(skb->data);
		real_size = get_sig_size(skb->data);
		put_user(offset, (uint32_t *)&sndrcv->offset);
		put_uptr(0, (unsigned long long *)&sndrcv->buffer);
		put_user(real_size, (uint32_t *)&sndrcv->real_size);
		break;
	}
	default:
		break;
...
#else
static int linx_recvmsg(struct socket *sock, struct msghdr *msg, size_t size, int flags)
#endif
{
	struct sock *sk = sock->sk;
	struct sk_buff *skb;
...
#ifdef ERRORCHECKS
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3,12,4))
	/* Since Linux 3.12.4 we can assume that msg.msg_name is either NULL or has
     * a minimum size of sizeof(struct sockaddr_storage). */
	...
	/* Receive the first datagram of the receive queue. */
	timeo_jiffies = sk->sk_rcvtimeo;
	skb = linx_recv_datagram(sk, flags, &ret, size, &timeo_jiffies);
	if (unlikely(ret != 0)) {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
			 (int __user *)(msg->msg_iter.iov->iov_base));
#else
	...
	/* Save a pointer to the signo, skb_copy_datagram_iter/iovec updates
	 * msg-msg_iov->iov_base while copying and we need the original. */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
	signop = (LINX_SIGSELECT *) msg->msg_iter.iov->iov_base;
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
		ret = copy_to_iter(skb->data, msg->msg_iter.iov->iov_len, &msg->msg_iter);
#else
		ret = copy_to_user(msg->msg_iov->iov_base, skb->data,
				msg->msg_iov->iov_len);
#endif
	} else {
	/* Copy the payload of the signal to the receive buffer. */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,19,0))
		ret = skb_copy_datagram_iter(skb, 0, &msg->msg_iter, size);
#else
		ret = skb_copy_datagram_iovec(skb, 0,
				msg->msg_iov, size);
#endif
	}
...
static int linx_ioctl_legacy_hunt(struct sock *sk,
				  unsigned long arg, int compat)
{
	LINX_SPID hunt_spid, spid = linx_sock_to_spid(sk);
	int err = 0;
	struct linx_hunt_legacy_param hunt_param;
	int hunt_param_size = sizeof(struct linx_hunt_legacy_param);

#ifdef CONFIG_64BIT
#ifdef CONFIG_COMPAT
	if (unlikely(compat))
		hunt_param_size = linx_compat_size(linx_hunt_legacy_param);
#endif
#endif
...
static int linx_ioctl_hunt(struct sock *sk, unsigned long arg)
{
	LINX_SPID hunt_spid, from, spid = linx_sock_to_spid(sk);
	int err = 0;
	void *signal = NULL;
	unsigned long sig, name;
	uint32_t offset, namelen;
	LINX_OSBUFSIZE sigsize;

	const struct linx_hunt_param __user *hunt_param =
			(struct linx_hunt_param *)arg;
	...
	get_uptr(sig, &hunt_param->sig);
	get_uptr(name, &hunt_param->name);
	get_user(offset, &hunt_param->offset);
	get_user(namelen, &hunt_param->namelen);
	get_user(sigsize, &hunt_param->sigsize);
	get_user(from, &hunt_param->from);

	if (sig == 0x0 && sigsize != 0) {
		signal = pool_offset_to_sig(offset);
	} else {
		signal = (void *)sig;
	}

	err = linx_hunt(sk, (char *)name, namelen, signal,
			sigsize, spid, from, &hunt_spid,
			LINX_FALSE);
	if (err < 0 && is_pool_signal(signal))
		free_mem(signal, sk);
	return err;
}

static int linx_ioctl_legacy_attach(struct sock *sk,
				    unsigned long arg, int compat)
{
	struct linx_attach_legacy_param attach_param;
	LINX_OSATTREF attref;
	int err;
	int attach_param_size = sizeof(struct linx_attach_legacy_param);

#ifdef CONFIG_64BIT
#ifdef CONFIG_COMPAT
	if (unlikely(compat))
		attach_param_size = linx_compat_size(linx_attach_legacy_param);
#endif
#endif
	...
	if (unlikely(compat)) {
		put_user(attref, (int __user *)
			 ((char *)(arg) +
			  linx_compat_offsetof(linx_attach_legacy_param,
					       attref)));
	} else
#endif
#endif
...
static int linx_ioctl_attach(struct sock *sk, unsigned long arg)
{
	LINX_SPID spid;
	LINX_OSBUFSIZE sigsize;
	unsigned long sig;
	...
	const struct linx_attach_param __user *attach_param =
			(struct linx_attach_param *)arg;

	linx_trace_enter(LINX_TRACEGROUP_AF_LINX, "%p, %lu", sk, arg);

	/* Copy data from user space. */
	get_uptr(sig, &attach_param->sig);
	get_user(sigsize, &attach_param->sigsize);
	get_user(spid, &attach_param->spid);
	get_user(offset, &attach_param->offset);
	...
	err = linx_attach(sk, spid, signal,
			  sigsize, &attref, LINX_FALSE);

	if (err != 0) {
		if (is_pool_signal(signal))
			free_mem(signal, sk);
		return err;
	}
				     ...
				     unsigned long arg, int compat)
{
	...
	/* Copy the structure from user space to kernel space. */
...
static int linx_ioctl_legacy_request_tmo(struct sock *sk,
					 unsigned long arg, int compat)
{
	struct linx_tmo_legacy_param tmo_param;
	LINX_OSTMOREF tmoref;
	int err;
	int tmo_param_size = sizeof(struct linx_tmo_legacy_param);
...
#ifdef CONFIG_COMPAT
	if (unlikely(compat))
		tmo_param_size = linx_compat_size(linx_tmo_legacy_param);
#endif
#endif
	...
	if (unlikely(compat)) {
		put_user(tmoref, (int __user *)
			 ((char *)(arg) +
			  linx_compat_offsetof(linx_tmo_legacy_param, tmoref)));
	} else
#endif
#endif
		put_user(tmoref, (int __user *)
...
static int linx_ioctl_request_tmo(struct sock *sk, unsigned long arg)
{
	LINX_OSTIME tmo;
	LINX_OSBUFSIZE sigsize;
	unsigned long sig;
	...
	const struct linx_tmo_param __user *tmo_param =
			(struct linx_tmo_param *)arg;

	linx_trace_enter(LINX_TRACEGROUP_AF_LINX, "%p, %lu", sk, arg);

	/* Copy data from user space. */
	get_uptr(sig, &tmo_param->sig);
	get_user(sigsize, &tmo_param->sigsize);
	get_user(tmo, &tmo_param->tmo);
	get_user(offset, &tmo_param->offset);
	...
	err = linx_request_tmo(sk, tmo,
			       signal, sigsize, &tmoref);
	if (err != 0) {
		if (is_pool_signal(signal))
			free_mem(signal, sk);
		return err;
	...
	put_user(tmoref,
			(int __user *)&((struct linx_tmo_param *)arg)->tmoref);
	return 0;
}

static int linx_ioctl_legacy_cancel_tmo(struct sock *sk,
					unsigned long arg, int compat)
{
...
static int linx_ioctl_cancel_tmo(struct sock *sk, unsigned long arg)
{
	struct linx_tmo_param tmo_param;
	int err;
	int tmo_param_size = sizeof(struct linx_tmo_param);
	...
	/* dig out the signal from pool? */
	err = linx_cancel_tmo(sk, tmo_param.tmoref);

	return err;
}

static int linx_ioctl_legacy_modify_tmo(struct sock *sk, unsigned long arg,
					int compat)
{
	struct linx_tmo_param tmo_param;
	int err;
...
static int linx_ioctl_modify_tmo(struct sock *sk, unsigned long arg)
{
	struct linx_tmo_param tmo_param;
	int err;
	int tmo_param_size = sizeof(struct linx_tmo_param);
	...
	/* dig out the signal from the pool? */
	err = linx_modify_tmo(sk, tmo_param.tmo, tmo_param.tmoref);
	return err;
}
...
int linx_mmap(struct file *f, struct socket *sock, struct vm_area_struct *vma)
{
	int ret;
	unsigned long pfn;
	char *pool;
	...
	pool = (char *)get_pool_base_addr();
	pool_size = ROUNDUP(get_pool_size() + get_pool_offset(), PAGE_SIZE);

	if (length != pool_size) {
		printk("error: called with wrong size, expected %Zd", pool_size);
		return -ENOMEM;
	}

	/* go to the first page */
	pool -= get_pool_offset();

	/* loop over all pages, map it page individually */
	while (length > 0) {
		pfn = vmalloc_to_pfn(pool);
...
static int linx_ioctl_alloc(struct sock *sk, unsigned long arg)
{
	...
	rv = copy_from_user(&alloc_param, (void *)arg,
			    sizeof(struct linx_alloc_param));
	if (0 != rv)
		return -EFAULT;

	pool = (char *)get_pool_base_addr();

	/* coml-findme: Get memory from pool specifying user requested size.
  	  Pool implementation rounds up the size according to the alignment */
	ptr = get_mem(alloc_param.size, sk, &real_size);

	if (ptr == NULL)
		return -ENOMEM;

	alloc_param.offset = (int)(ptr - pool);
	alloc_param.size = real_size;


	put_user(alloc_param.offset,
		 (int __user *) ((char *)(arg) +
				 offsetof(struct linx_alloc_param, offset)));
	put_user(alloc_param.size,
		 (int __user *) ((char *)(arg) +
				 offsetof(struct linx_alloc_param, size)));
	return rv;
}


static int linx_ioctl_free(struct sock *sk, unsigned long arg)
{
	struct linx_free_param free_param;
	char *ptr;
	char *pool;

	if (0 != copy_from_user(&free_param, (void *)arg,
				sizeof(struct linx_free_param))) {
		return -EFAULT;
	}

	pool = (char *)get_pool_base_addr();
	ptr = (char *)(pool + free_param.offset);

	free_mem(ptr, sk);
	return 0;
}

static int linx_ioctl_pool_info(struct sock *sk, unsigned long arg)
{
	uint32_t pool_size;
	uint32_t headroom;
	uint16_t offset;

	const struct linx_pool_info __user *pool_info =
		(struct linx_pool_info *)arg;

	pool_size = get_pool_size();
	headroom = get_headroom_size();
	offset = get_pool_offset();

	put_user(pool_size, (uint32_t*)&pool_info->size);
	put_user(headroom, (uint32_t *)&pool_info->headroom);
	put_user(offset, (uint16_t *)&pool_info->offset);

	return 0;
}

static int linx_ioctl_pool_statistics(struct sock *sk, unsigned long arg)
{
	size_t pool_size;
	uint32_t headroom, free_size, total_overhead;
	uint16_t alig, lists_no, offset;
	struct linx_pool_list_info *list_info;
	char *pool;
	int idx;
	...
	list_info = linx_kmalloc(sizeof(struct linx_pool_list_info) *
							MAX_POOL_LISTS);
	...
	pool_size = get_pool_size();
	headroom = get_headroom_size();
	free_size = get_pool_free_size();
	total_overhead = get_pool_overhead();
	alig = get_pool_alignment();
	lists_no = get_pool_lists_info(list_info, MAX_POOL_LISTS);
	offset = get_pool_offset();

	put_uptr(((unsigned long *)pool)[0], (unsigned long *)&pool_info->base_addr);
	put_user(pool_size, (unsigned long *)&pool_info->size);
	put_user(headroom, (unsigned long *)&pool_info->headroom);
	put_user(free_size, (unsigned long *)&pool_info->free);
	put_user(total_overhead, (unsigned long *)&pool_info->overhead);
	put_user(alig, (unsigned short *)&pool_info->alignment);
	put_user(lists_no, (unsigned short *)&pool_info->num_lists);
	put_user(offset, (unsigned short *)&pool_info->offset);

	for(idx = 0; idx < lists_no; idx++) {
		put_user(list_info[idx].block_size,
			 (unsigned long *)
			 &pool_info->list[idx].block_size);

		put_user(list_info[idx].allocated,
			 (unsigned long *)
			 &pool_info->list[idx].allocated);

		put_user(list_info[idx].available,
			 (unsigned long *)
			 &pool_info->list[idx].available);
	}

	linx_kfree(list_info);
...
static int linx_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	case LINX_IOCTL_HUNT:
		err = linx_ioctl_hunt(sk, arg);
		break;

	case LINX_IOCTL_LEGACY_HUNT:
		err = linx_ioctl_legacy_hunt(sk, arg, LINX_FALSE);
		break;

	case LINX_IOCTL_ATTACH:
		err = linx_ioctl_attach(sk, arg);
		break;

	case LINX_IOCTL_LEGACY_ATTACH:
		err = linx_ioctl_legacy_attach(sk, arg, LINX_FALSE);
		break;
	...
	case LINX_IOCTL_REQUEST_TMO:
		err = linx_ioctl_request_tmo(sk, arg);
		break;

	case LINX_IOCTL_CANCEL_TMO:
		err = linx_ioctl_cancel_tmo(sk, arg);
		break;

	case LINX_IOCTL_MODIFY_TMO:
		err = linx_ioctl_modify_tmo(sk, arg);
		break;

	case LINX_IOCTL_LEGACY_REQUEST_TMO:
		err = linx_ioctl_legacy_request_tmo(sk, arg, LINX_FALSE);
		break;

	case LINX_IOCTL_LEGACY_CANCEL_TMO:
		err = linx_ioctl_legacy_cancel_tmo(sk, arg, LINX_FALSE);
		break;

	case LINX_IOCTL_LEGACY_MODIFY_TMO:
		err = linx_ioctl_legacy_modify_tmo(sk, arg, LINX_FALSE);
		break;
	...
	case LINX_IOCTL_LEGACY_2_SEND:
		err = linx_ioctl_legacy_2_send(sk, arg);
		break;

	case LINX_IOCTL_LEGACY_2_RECEIVE:
		err = linx_ioctl_legacy_2_receive(sk, arg);
		break;
	...
	case LINX_IOCTL_ALLOC:
		err = linx_ioctl_alloc(sk, arg);
		break;

	case LINX_IOCTL_FREE:
		err = linx_ioctl_free(sk, arg);
		break;

	case LINX_IOCTL_POOL_INFO:
		err = linx_ioctl_pool_info(sk, arg);
		break;
	case LINX_IOCTL_POOL_STATISTICS:
		err = linx_ioctl_pool_statistics(sk, arg);
		break;

#ifdef CONFIG_64BIT
#ifdef CONFIG_COMPAT
	case LINX_IOCTL_HUNT_32:
		err = linx_ioctl_legacy_hunt(sk, arg, LINX_TRUE);
		break;

	case LINX_IOCTL_ATTACH_32:
		err = linx_ioctl_legacy_attach(sk, arg, LINX_TRUE);
		break;
	...
	case LINX_IOCTL_REQUEST_TMO_32:
		err = linx_ioctl_legacy_request_tmo(sk, arg, LINX_TRUE);
		break;

	case LINX_IOCTL_CANCEL_TMO_32:
		err = linx_ioctl_legacy_cancel_tmo(sk, arg, LINX_TRUE);
		break;

	case LINX_IOCTL_MODIFY_TMO_32:
		err = linx_ioctl_legacy_modify_tmo(sk, arg, LINX_TRUE);
		break;
#endif
#endif
	default:
...
#elif(LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,32) && ! defined (RHEL_COMPAT))
