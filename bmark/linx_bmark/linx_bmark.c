...
while ((c = getopt(argc, argv, "S?p:ac:n:m:b:i:s:e:Alt:qP")) != -1) {
	...
	case 't':
		...
		if(throughput_instances > 256)
		{
			throughput_instances = 256;   
		}
		if(throughput_instances < 0)
		{
