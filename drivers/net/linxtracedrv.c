...
#endif
{
	...
	DBGPRINTK("TRACEIF len:%d head:%p data:%p tail:%u end:%u\n",
...
int linxtrace_header_cache(const struct neighbour *neigh, struct hh_cache *hh, __be16 type)
...
static int __init linxtrace_init_one(int index)
{
	...
	dev_linxtrace = alloc_netdev(sizeof(struct net_device_stats),
					"linx%d", NET_NAME_UNKNOWN, linxtrace_setup);
#else
	dev_linxtrace = alloc_netdev(sizeof(struct net_device_stats),
